...
const CybersecurityTestimonial = () => {
  ...
  const testimonials = [
    {
      ...
      content:
        "This cybersecurity course completely transformed my career. The hands-on labs and real-world scenarios prepared me for actual threats I face daily. Best investment I've made!",
    ...
    },
    {
      ...
      content:
        "The ethical hacking modules were incredibly comprehensive. I went from beginner to landing my dream job in just 6 months. The instructors are industry experts who know their craft.",
    ...
    },
    {
      ...
      content:
        "Outstanding course content covering everything from network security to incident response. The certification helped me secure a 30% salary increase within three months.",
    ...
    },
    {
      ...
      content:
        "The practical approach to teaching complex concepts made learning enjoyable. The community support and mentorship were invaluable throughout my entire journey.",
    ...
    },
    {
      ...
      role: "SOC Analyst",
      content:
        "From threat detection to malware analysis, this course covered it all. The 24/7 lab access allowed me to practice at my own pace. Highly recommended for anyone serious!",
    ...
    },
    {
      ...
      content:
        "The instructors' real-world experience shines through every lesson. I gained practical skills that I use every single day in my role as a security engineer. Worth every penny!",
  ...
  const CardBody = ({ t, isCenter = true }) => (
    <div className="flex flex-col h-full">
      {/* Photo Section */}
      <div
        className="relative overflow-hidden bg-[#0d0d0d] shrink-0"
      ...
      <div className={`flex-1 flex flex-col ${isMobile ? "p-4" : "p-5"}`}>
        ...
        {/* Quote */}
        <p
          className={`italic mb-4 flex-1 ${isMobile ? "text-[0.9rem] leading-[1.6]" : "text-[0.85rem] leading-[1.7]"}`}
          style={{
        ...
        <div className="flex items-center gap-3 mt-auto pt-3 border-t border-[#1a1a1a]">
          ...
          <div className="flex-1 min-w-0">
            <div
              className={`font-bold truncate ${isMobile ? "text-[0.9rem]" : "text-[0.83rem]"}`}
              style={{ color: isCenter ? "#fff" : "#444" }}
            >
              {t.name}
            </div>
            <div
              className={`font-semibold ${isMobile ? "text-[0.75rem]" : "text-[0.7rem]"}`}
          ...
          <div
            className={`shrink-0 ${isMobile ? "text-[0.65rem]" : "text-[0.61rem]"}`}
            style={{ color: "#4a4a4a" }}
          >
            {t.date}
          </div>
    ...
    <div className="w-full bg-[#080808] overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 t-dm">
        ...
        <div className="text-center mb-6 sm:mb-10">
          <span className="t-tag mb-3 inline-block">
          ...
          <p
            className={`text-[#4a4a4a] max-w-[440px] mx-auto leading-[1.7] ${isMobile ? "text-[0.85rem] px-4" : "text-sm sm:text-[0.92rem]"}`}
          >
            Join thousands of professionals who launched or leveled up their
        ...
        {/* Stats */}
        <div
          className={`grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-8 sm:mb-12 ${isMobile ? "px-2" : ""}`}
        >
          {[
            ...
            <div key={i} className="t-stat">
              <div
                className={`t-bebas leading-none mb-1 ${isMobile ? "text-[1.3rem]" : "text-[1.5rem] sm:text-[1.9rem]"} ${i % 2 === 0 ? "text-white" : "text-red-600"}`}
              >
                {s.val}
              </div>
              <div
                className={`text-[#4a4a4a] font-semibold tracking-[.8px] uppercase ${isMobile ? "text-[0.6rem]" : "text-[0.62rem]"}`}
              >
                {s.lbl}
              </div>
            </div>
        ...
        {!isMobile && (
          <div className="relative">
            ...
            <div className="flex justify-center items-center gap-4 mt-6">
              <button type="button" className="t-nav" onClick={goPrev}>
                ‹
              </button>
              <div className="flex gap-2 items-center">
                {testimonials.map((_, i) => (
              ...
              <button type="button" className="t-nav" onClick={goNext}>
                ›
              </button>
            </div>
          </div>
        ...
        {/* Avatar Strip */}
        <div
          className={`mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-[#141414] ${isMobile ? "px-4" : ""}`}
        >
          <p
            className={`text-center font-bold uppercase mb-4 ${isMobile ? "text-[0.65rem] tracking-[2px] text-[#2a2a2a]" : "text-[0.62rem] tracking-[2.5px] text-[#2a2a2a]"}`}
          >
            Browse All Stories
          </p>
          ...
          <p
            className={`text-center mt-3 font-bold text-red-600 ${isMobile ? "text-[0.85rem]" : "text-[0.78rem]"}`}
          >
            {current.name}
            ...
            {current.company && (
              <span className="text-[#2a2a2a] hidden sm:inline">
                {" "}
                · {current.company}
              </span>
            )}
        ...
        {/* CTA Banner */}
        <div
          className={`mt-8 sm:mt-12 bg-[#0a0a0a] border border-[#1a1a1a] rounded-2xl ${isMobile ? "p-5" : "p-5 sm:p-8 lg:p-10"} flex flex-col sm:flex-row items-center justify-between gap-5 relative overflow-hidden`}
        >
          <div className="absolute top-0 left-0 w-[3px] h-full bg-gradient-to-b from-red-600 to-transparent" />
          <div className={`${isMobile ? "text-center" : ""}`}>
            <h3
              className={`t-bebas text-white tracking-[1px] mb-1 ${isMobile ? "text-[1.4rem]" : "text-[1.55rem] sm:text-[2rem]"}`}
            >
              YOUR SUCCESS STORY STARTS TODAY
            </h3>
            <p
              className={`text-[#4a4a4a] leading-[1.6] ${isMobile ? "text-[0.85rem]" : "text-[0.82rem]"}`}
            >
              Join 12,400+ students already transforming their cybersecurity
              careers.
            </p>
          </div>

          {/* ✅ Wrapper removes the marginTop from FormModal's internal button */}
          <div className="flex-shrink-0  z-20 [&_button]:!mt-0">
            <FormModal label="Enroll Now" />
          </div>
        </div>
      </div>
    </div>
